...
private extension AuthenticationOperation
{
    ...
    func fetchCertificate(for team: ALTTeam, session: ALTAppleAPISession, completionHandler: @escaping (Result<ALTCertificate, Swift.Error>) -> Void)
    {
        ...
        {
            guard let certificate = certificates.first(where: { $0.machineName?.starts(with: "Kekstore") == true }) ?? certificates.first else { return completionHandler(.failure(AuthenticationError.noCertificate)) }
